import { ct as throttle, f as checkTwoRectIsOverlap, i as bfsWalk } from "./utils-DKbIT76G.js";
import { t as AutoMove } from "./AutoMove-CBoxW4Rg.js";
//#region node_modules/simple-mind-map/src/plugins/Select.js
var Select = class {
	constructor({ mindMap }) {
	...
	onMousedown(e) {
		...
		if (!(e.ctrlKey || e.metaKey) && (useLeftKeySelectionRightKeyDrag ? e.which !== 1 : e.which !== 3)) return;
	...
	onMousemove(e) {
		...
		this.mouseMoveX = x;
		this.mouseMoveY = y;
		if (Math.abs(x - this.mouseDownX) <= 10 && Math.abs(y - this.mouseDownY) <= 10) return;
		this.autoMove.clearAutoMoveTimer();
		this.autoMove.onMove(e.clientX, e.clientY, () => {
			this.isSelecting = true;
			if (this.rect) this.rect.plot([
	...
	checkTriggerNodeActiveEvent() {
		...
		if (!isNumChange) for (let i = 0; i < this.cacheActiveList.length; i++) {
		...
		if (isNumChange || isNodeChange) this.mindMap.renderer.emitNodeActiveEvent();
	}
	createRect(x, y) {
		if (this.rect) this.rect.remove();
		this.rect = this.mindMap.svg.polygon().stroke({ color: "#0984e3" }).fill({ color: "rgba(9,132,227,0.3)" }).plot([[x, y]]);
	}
	checkInNodes() {
		...
		const check = (node) => {
			...
			if (checkTwoRectIsOverlap(minx, maxx, miny, maxy, left, right, top, bottom)) {
				if (node.getData("isActive")) return;
