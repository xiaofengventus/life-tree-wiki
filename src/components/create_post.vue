<template>
  <section class="create-post-page">
    <div class="create-post-container">
      <div>
        <h1>{{ isEditing ? "编辑帖子" : "创建新帖子" }}</h1>
        <p
          class="draft-save-state"
          :class="{ error: draftAutosave.errorMessage.value }"
        >
          {{ draftStatusText }}
        </p>
      </div>
      <div class="draft-actions">
        ...
        <RouterLink to="/drafts">打开草稿箱</RouterLink>
      </div>
    </div>
    <p v-if="isEditing" class="editing-hint">
      {{
        isPrivateWork
          ? "正在编辑仅自己可见的文章；公开发布后才会建立第一条正式版本记录。"
    ...
    <form class="create-post-form" @submit.prevent="submitPost('PUBLIC')">
      ...
      <div class="content-area">
        <label for="post-title" class="content-title-label">标题</label>
        <input
          id="post-title"
          v-model="title"
          type="text"
      ...
      <!-- 内容区下方：封面和元数据 -->
      <div class="post-metadata-section">
      ...
      <div class="submission-section">
        ...
        <div class="submission-options">
          <label for="post-type">文章类型</label>
          <select id="post-type" v-model="postType">
            <option value="science">科普</option>
            <option value="news">新闻</option>
            <option value="">未分类</option>
          </select>
          <small>类型会显示在专栏列表和文章信息栏中</small>
        </div>
      ...
      <small v-if="!isEditing || isPrivateWork" class="private-save-hint">
...
function syncSidebarCardsFromEditor() {
  ...
  sidebarCards.value = sidebarCards.value.map((card, index) => {
    const source = editorCards.get(card.id);
    if (!source || !bodyIds.has(card.id)) return card;
    const next = {
      ...card,
      title: source.title,
      color: source.color,
      ...
      rows: source.rows.map((row, rowIndex) => ({
        ...(card.rows[rowIndex] || { id: `${card.id}-row-${rowIndex + 1}` }),
        ...row,
      })),
    };
...
async function handleExport(format) {
  ...
  try {
    importExportLoading.value = true;
    importExportMessage.value = `正在导出${format === "docx" ? "Word 文档" : format === "md" ? "Markdown" : "富文本"}……`;
...
.editor-wrapper :deep(.w-e-text-container [data-slate-editor] h1),
.editor-wrapper :deep(.w-e-text-container [data-slate-editor] h2),
.editor-wrapper :deep(.w-e-text-container [data-slate-editor] h3) {
  color: #20242a;
...
.editor-wrapper :deep(.w-e-text-container [data-slate-editor] h1) { margin: 2.2em 0 0.85em; font-size: 1.7rem; }
.editor-wrapper :deep(.w-e-text-container [data-slate-editor] h2) {
...
.editor-wrapper :deep(.w-e-text-container [data-slate-editor] h3) { margin: 1.8em 0 0.7em; font-size: 1.13rem; }

.editor-wrapper :deep(.w-e-text-container [data-slate-editor] blockquote) {
  margin: 1.7em 0;
  padding: 13px 18px;
